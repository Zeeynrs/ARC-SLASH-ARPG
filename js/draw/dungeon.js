...
function drawSlimePuddles(ctx) {
    ...
    slimePuddles.forEach(puddle => {
        const alpha = Math.min(0.65, puddle.life / 60);
        ctx.save();
        ctx.fillStyle = `rgba(16, 185, 129, ${alpha})`;
        ctx.beginPath();
        ctx.ellipse(puddle.x, puddle.y, puddle.radius, puddle.radius * 0.55, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = `rgba(52, 211, 153, ${alpha * 1.2})`;
        ctx.lineWidth = 1.5;
        ctx.stroke();
        // Little toxic bubble
        const bubbleY = puddle.y - 2 + Math.sin(Date.now() * 0.006 + puddle.x) * 3;
        ctx.fillStyle = `rgba(255, 255, 255, ${alpha * 0.8})`;
        ctx.beginPath();
        ctx.arc(puddle.x + 4, bubbleY, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
    });
}
...
function drawDungeonFloor() {
    ...
    // Lingering Slime Toxic Puddles on the floor
    drawSlimePuddles(ctx);
